...
CREATE TABLE IF NOT EXISTS `naver_movie`.`review_comment` (
  ...
  `write_time` VARCHAR(45) NULL,
  PRIMARY KEY (`reviewcomment_id`, `review_id`, `movie_code`),
  CONSTRAINT `fk_review_comment_review1`
    FOREIGN KEY (`review_id` , `movie_code`)
    REFERENCES `naver_movie`.`review` (`review_id` , `movie_code`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `fk_review_comment_review1_idx` ON `naver_movie`.`review_comment` (`review_id` ASC, `movie_code` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `naver_movie`.`country` (
  ...
  `movie_code` INT NOT NULL,
  `name` VARCHAR(45) NULL,
  PRIMARY KEY (`country_code`, `movie_code`),
...
CREATE INDEX `fk_country_movie1_idx` ON `naver_movie`.`country` (`movie_code` ASC) VISIBLE;
